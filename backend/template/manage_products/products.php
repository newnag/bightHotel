<?php
  ...
  $getpost['product_cate'] = isset($_GET['bycate']) ? $_GET['bycate'] : "";
  $getpost['pin'] = isset($_GET['bypin']) ? $_GET['bypin'] : "";
  $search_text = isset($_GET['search']) ? $_GET['search'] : ""; 
  $all_posts = $mydata->get_post_product($getpost);
  $product_cate = $mydata->get_product_cate();
  $product_brand = $mydata->get_product_brand(); 
  // $product_cate_all = $mydata->get_product_cate_all();
  $product_cate_all = $product_cate;
?>
<style>
          ...
          <div class="row" style="margin-top:10px;">
             <div class="col-md-12" style="text-align:center">
              <label><?php echo "หมวดหมู่สินค้า";?> : </label>
                <select class="form-control" id="product_type" style="width: 300px; display: inline-block; margin: 0 10px 5px 5px;">
                  <option value="">ทั้งหมด</option>
                  <?php foreach ($product_cate_all as $value) {
                    if($value['product_cate_id'] == $_GET['bycate']){
                      echo " <option value=\"".$value['product_cate_id']."\" selected>".$value['product_cate_name']."</option> ";
                    }else{
              ...
              <div class="attachment-block clearfix" <?php if ($post['display'] == 'no') {echo 'style="opacity: 0.6;"';} ?> >
                ...
                <div class="content-info">
                  <h1>
                    ...
                    <?php echo $post['title']; ?> <span style="color:mediumseagreen;margin-left:10px;"> ราคา <?php echo number_format($post['saleprice']); ?>  </span>
                    <?php
                      if ($post['display'] == 'no') {
                    ...
                    ?>
                    <label class = "" style="color:red">id : <?php echo $post['id']; ?></label>
                    <?php } ?>
                  </h1>
                  <p class="text-category">
                    หมวดหมู่ <?=$mydata->get_product_cate_name($post['product_cate_id'])?>
                  </p>
                  <!-- <p class="text-category">
...
<script src="<?php echo SITE_URL; ?>js/pages/product_sel/product_sel.js?v=<?php echo date('s');?>"></script>
<script src="<?php echo SITE_URL; ?>js/pages/product_sel/addproduct_sel.js?v=<?php echo date('s');?>"></script>
<script src="<?php echo SITE_URL; ?>js/pages/product_sel/editproduct_sel.js?v=<?php echo date('s');?>"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contents/hidebtn.js?v=<?php echo date('s');?>"></script>
