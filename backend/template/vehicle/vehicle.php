<?php
  ...
  $location = $mydata->get_location_route();
?>
        ...
        <div class="box box-primary">
          ...
          <div class="categorybox-controls">
            ...
            ?>
              ...
              foreach ($category as $value) {
                  if ($value['main_page'] != 'yes') {
                    if($_REQUEST['bycate']==$value['cate_id']) {
                      $select = 'SELECTED';
                    }else {
                      $select = '';
                    }
                    echo '
                    <option value="'.$value['cate_id'].'" '.$select.'>'.$value['cate_name'].'</option>';
                  }
                }
              ?>
            </select>
            ...
            <div class="pull-right">
              <button type="button" class="btn btn-attree" data-toggle="modal" data-target="#modalAddContent"><i class="fa fa-plus"></i> เพิ่มข้อมูล</button>
            </div>
            <?php
              }else {
            ?>
            ...
            ?>
              ...
              >
                ...
                <div class="content-info">
                  ...
                  <p class="text-category"><i class="fa fa-folder-open-o" aria-hidden="true"></i> <?php echo substr($mydata->getcontentcate($a['category']),0,-2); ?> </p>
                  <p class="text-category"><i class="fa fa-map-marker" aria-hidden="true"></i> <?= $location[$a['h1']]['info_title'].' to '.$location[$a['h2']]['info_title']; ?> </p>
                  <p class="text-datetime"><i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo $a['date_created']; ?>, <i class="fa fa-globe" aria-hidden="true"></i> <?php echo substr($a['lang_info'],1); ?></p>
                </div>
                <div class="content-button pull-right">
                  <?php
                    if(strpos($a['lang_info'],$_SESSION['backend_language'])){
                  ?>
                    <button type="button" class="btn btn-success margin-r-10 edit-content" data-id="<?= $a['id']; ?>" data-type="edit" data-toggle="modal" data-target="#modalEditContent">
                      <i class="fa fa-pencil-square-o"></i> แก้ไข
                    </button>

                    <button type="button" class="btn btn-danger delete-content" data-id="<?= $a['id']; ?>">
                      <i class="fa fa-trash-o" aria-hidden="true"></i> ลบ
                    </button>
                  <?php
                    }else {
                  ?>
                    <button type="button" class="btn btn-primary margin-r-10 edit-content" data-id="<?= $a['id']; ?>" data-type="add" data-toggle="modal" data-target="#modalEditContent">
...
<?php
  include 'template/vehicle/addvehicle.php';
  include 'template/vehicle/editvehicle.php';
?>
<!-- css -->
...
<script src="<?php echo SITE_URL; ?>js/pages/js-vehicle/vehicle.js"></script>
<script src="<?php echo SITE_URL; ?>js/pages/js-vehicle/addvehicle.js"></script>
<script src="<?php echo SITE_URL; ?>js/pages/js-vehicle/editvehicle.js"></script>
