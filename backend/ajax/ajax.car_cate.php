...
require_once('../classes/class.car_cate.php');
...
if(isset($_REQUEST['action'])) {
    ...
    switch($_REQUEST['action']){
		...
		case'getcategoryedit':
			...
			$sql = "SELECT * FROM car_cate WHERE cate_id = '".$_REQUEST['id']."' ORDER BY FIELD( defaults,  'yes')DESC";
        ...
        break;
        case 'addcategory':
			$sql = " SELECT COUNT(url) AS 'count_url' FROM car_cate WHERE url = '".$_REQUEST['slug']."' ";
        	...
        	if ($count_url == 0) {
				if ($_REQUEST['parentId']==0) {
					...
					$sql = "SELECT level FROM car_cate WHERE cate_id='".$_REQUEST['parentId']."'";
					$level = $dbcon->fetch_assoc($sql);
					$level++;
				}

				$sql = "SELECT MAX(priority) pmax FROM car_cate WHERE parent_id='".$_REQUEST['parentId']."'";
				...
				$sql = "SELECT MAX(cate_id) imax FROM car_cate";
