...
function get_all_category() {
  var url = url_ajax_request + "ajax/ajax.category.php",
...
$("#search-cate").on("keyup", function(event){ 
  if(event.keyCode == 13){
    ...
    var new_url = decodeURIComponent(getQueryVariable('page=category', 'search', $("#search-cate").val()));
...
function uploadimages(cateId,action) {
  ...
  $.ajax({
    url: url_ajax_request + "ajax/ajax.category.php",
    type: 'POST',
    data: formdata,
    processData: false,
    contentType: false,
