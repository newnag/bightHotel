...
$("#add-slug").keyup((e) => {
    let slug = $("#add-slug").val();
    $.ajax({
      type:"post",
      dataType:"json",
      url:"ajax/ajax.contentWeb.php",
      data:{action:"checkurl",slug},
      success:function(data){
        // console.log(data);
        if(data.data.message == "url_already_exists"){
          // console.log('failed');
...
function add_content(data) {
  var url = url_ajax_request + "ajax/ajax.contentWeb.php",
            dataSet = data;
  $.ajax({
    type: "POST",
    url: url,
...
function validate_add_content(data) {
  ...
  if(pattern.test(priority.val())){
    $('.form-add-priority').removeClass("has-error");
    $('.add-priority-error').css('display','none');
    // console.log("valid");
  }else{
    $('.form-add-priority').addClass("has-error");
    $('.add-priority-error').text('has-error');
    $('.add-priority-error').css('display','block');
    // console.log("invalid");
    return false;
  }
...
$('#add-priority').keyup(function(e){
  var v = $(this).val();
  console.log(v);
  validate_add_content_priority(e,v);
});


function validate_add_content_priority(e,v){
