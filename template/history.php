<div class="history-page">
    <div class="title">
        <h1>ประวัติการจอง</h1>
        <span>โรงแรมสไตล์โมเดิร์นคลาสิค แห่งแรกในจังหวัดขอนแก่น</span>
    </div>

    <div class="history-page-zone">
        <div class="grid-item">
            <a href="">
                <div class="item">
                    <div class="table-head">
                        <span>วัน</span>
                        <span>ประเภทห้อง</span>
                        <span class="discount">ส่วนลด</span>
                        <span class="group-price">ราคา</span>
                        <span class="status">สถานะ</span>
                        <span class="delete">ลบ</span>
                    </div>
                    <div class="table-body">
                        <span>22 มิ.ย. 2563 - 23 มิ.ย. 2563</span>
                        <div class="group-nameroom">
                            <span>Triple Room</span>
                            <span>Deluxe Room</span>
                        </div>
                        <span class="discount">-</span>
                        <div class="group-price">
                            <span>1,400</span>
                            <span>2,400</span>
                        </div>
                        <div class="status">
                            <span class="non-pay">ยังไม่ชำระ</span>
                        </div>
                        <span class="delete">ลบ</span>
                    </div>
                    <div class="table-end">
                        <span>2 วัน 1 คืน</span>
                        <span>3,800</span>
                    </div>
                </div>
            </a> 

            <a href="">
                <div class="item">
                    <div class="table-head">
                        <span>วัน</span>
                        <span>ประเภทห้อง</span>
                        <span class="discount">ส่วนลด</span>
                        <span class="group-price">ราคา</span>
                        <span class="status">สถานะ</span>
                        <span class="delete">ลบ</span>
                    </div>
                    <div class="table-body">
                        <span>22 มิ.ย. 2563 - 23 มิ.ย. 2563</span>
                        <div class="group-nameroom">
                            <span>Triple Room</span>
                            <span>Deluxe Room</span>
                        </div>
                        <span class="discount">-</span>
                        <div class="group-price">
                            <span>1,400</span>
                            <span>2,400</span>
                        </div>
                        <div class="status">
                            <span class="prepare">รอตรวจสอบ</span>
                        </div>
                        <span class="delete">ลบ</span>
                    </div>
                    <div class="table-end">
                        <span>2 วัน 1 คืน</span>
                        <span>3,800</span>
                    </div>
                </div>
            </a>
            
            <a href="">
                <div class="item">
                    <div class="table-head">
                        <span>วัน</span>
                        <span>ประเภทห้อง</span>
                        <span class="discount">ส่วนลด</span>
                        <span class="group-price">ราคา</span>
                        <span class="status">สถานะ</span>
                        <span class="delete">ลบ</span>
                    </div>
                    <div class="table-body">
                        <span>22 มิ.ย. 2563 - 23 มิ.ย. 2563</span>
                        <div class="group-nameroom">
                            <span>Triple Room</span>
                            <span>Deluxe Room</span>
                        </div>
                        <span class="discount">-</span>
                        <div class="group-price">
                            <span>1,400</span>
                            <span>2,400</span>
                        </div>
                        <div class="status">
                            <span class="success">ยืนยันแล้ว</span>
                        </div>
                        <span class="delete">ลบ</span>
                    </div>
                    <div class="table-end">
                        <span>2 วัน 1 คืน</span>
                        <span>3,800</span>
                    </div>
                </div>
            </a>    
        </div>
    </div>
</div>